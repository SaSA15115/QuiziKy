...
import { createQuizInDB, getTeacherQuizzesWithAttempts } from '../lib/quizStorage';
...
const TeacherDashboard = () => {
  ...
  const loadQuizzes = async () => {
    if (!user?.id) return;
    try {
      const databaseQuizzes = await getTeacherQuizzesWithAttempts(user.id);
      setQuizzes(databaseQuizzes.map((quiz) => ({
        id: quiz.id,
        title: quiz.title,
        description: quiz.description || '',
        questions: Array.from({ length: quiz.questionCount }, (_, index) => ({ id: index, question: '', options: [], correct: 0, type: 'text' as const })),
        createdAt: new Date(quiz.created_at),
        timeLimit: quiz.time_limit ? Math.floor(quiz.time_limit / 60) : undefined,
        shareLink: `${window.location.origin}/quiz/shared/${quiz.id}`,
        ...
        isActive: quiz.is_public,
        teacher_id: quiz.teacher_id,
        is_public: quiz.is_public,
      })));
    } catch (error) {
      // A failed RLS/query must not be presented as an empty attempt list.
      console.error('Error loading teacher quizzes and attempts:', error);
      setQuizzes([]);
    }
  };
