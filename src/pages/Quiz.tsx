...
const Quiz = () => {
  ...
  const handleComplete = async () => {
    ...
    const score = finalAnswers.reduce((acc, answer, index) => {
      return acc + (answer === questions[index]?.correct ? (questions[index]?.points || 1) : 0);
    }, 0);
    ...
    if (user?.id) {
      try {
        ...
        if (!quizId) {
          throw new Error('No quiz ID found for saving attempt');
        } else {
        ...
        alert('Your attempt could not be saved: ' + (error instanceof Error ? error.message : 'Unknown error'));
        setIsCompleted(false);
        isSubmitting.current = false;
        return;
      }
    } else {
      alert('Please sign in as a student before submitting so your attempt can be saved.');
      setIsCompleted(false);
      isSubmitting.current = false;
      return;
    }
